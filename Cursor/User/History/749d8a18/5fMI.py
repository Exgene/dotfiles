...
def _set_env(var: str):
    if not os.environ.get(var):
        os.environ[var] = os.getenv(var)  # type: ignore
...
# Prompt Instructions
router_instructions = """You are an expert at routing a user question to either the healthcare agent or the general agent.
Route questions related to healthcare to the healthcare agent and all other questions to the general agent.
Return JSON with single key, datasource, that is 'healthcare' or 'general' depending on the question.
"""
...
def route_question(state):
    """Route question to healthcare or general agent"""
    # Determine the route based on the content of the chat message
    if "healthcare" in state["chat_message"].lower():
        return {"route": "healthcare"}
    else:
...
def generate(state):
    ...
    # Set the URL based on the route
    if route == "healthcare":
        url = "http://localhost:3000/generate/healthcare"
    else:
        url = "http://localhost:3000/generate/general"

    # Make a request to the appropriate agent
    response = requests.post(url, json={"message": chat_message})
    generation = response.json().get("response", "No response generated.")
