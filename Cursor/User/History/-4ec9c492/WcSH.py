...
class WhatsappClient:
    ...
    async def chat_pane_scroll_up(self):
        ...
        # TODO:
        # Check whether the chat panel is open or not
        # Pass amount of time it should run (scroll up)
    ...
    # Need a function like this to obtain new and unread messages
    ...
    # <----------------------------------------------------> #
    # A while loop to perpetually fetch new and unread messages
    # Also refer x-whatsapp.ipynb and x-whatsap-qr.ipynb

    async def extract_message_details(self, message):
        logger.info("Extracting message details.")
        name_element = await message.query_selector("div._ak8q > div > span")
        ...
        logger.info(f"Extracted time: {time_text}")

        message_element = await message.query_selector("div._ak8k > span > span")
        message_text = await message_element.inner_text() if message_element else "N/A"
        logger.info(f"Extracted message: {message_text}")

        return {"name": name, "time": time_text, "message": message_text}

    async def fetchLatestMessage(self):
        logger.info("Fetching the latest message.")
        selector = '//*[@id="pane-side"]/div[2]/div/div/child::div'
        await self.page.wait_for_selector(selector)
        list_items = await self.page.query_selector_all(selector)
                ...
                if "matrix" in transform:
                    ...
                    if len(parts) == 6:
                        ...
                        if translate_y == 0:
                            ...
                            return await self.extract_message_details(item)
                elif "translateY(0px)" in transform:
                    logger.info("Div with translateY(0px) found.")
                    return await self.extract_message_details(item)

        logger.info("Desired div not found.")
        return None

    async def onNewMessageWhile(self, callbackFunction):
        logger.info("Starting to listen for new messages.")
        new_message = await self.fetchLatestMessage()
            ...
            try:
                ...
                if new_message:
                    if not (
                        ...
                        name = new_message.get("name", None)
                        messages = await self.getChatHistory(10)
                        logger.info(f"New message received: {messages}")
                        await callbackFunction(new_message)
