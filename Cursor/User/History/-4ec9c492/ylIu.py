    ...
    async def extract_chat_details_from_side_pane(self):
        ...
        if chat_list_div:
            ...
            for chat in chat_list_div:
                name = await chat.query_selector('span[dir="auto"]').inner_text()
                recent_message = await chat.query_selector(
                    'span[dir="auto"]'
                ).inner_text()
                time = await chat.query_selector('span[dir="auto"]').inner_text()
