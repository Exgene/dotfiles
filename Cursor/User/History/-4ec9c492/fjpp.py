...
class WhatsappClient:
    ...
    async def extract_chat_details_from_side_pane(self):
        ...
        """
        chat_list_div = await self.page.locator('div[aria-label="Chat list"]')
        if chat_list_div:
            logger.info("Chat list div found.")
            for chat in chat_list_div:
                name = await chat.locator('span[dir="auto"]').inner_text()
                recent_message = await chat.locator('span[dir="auto"]').inner_text()
                time = await chat.locator('span[dir="auto"]').inner_text()
                unread_messages = await chat.locator('span[dir="auto"]').inner_text()
                logger.info(
                    f"Chat: {name}, Recent message: {recent_message}, Time: {time}, Unread messages: {unread_messages}"
                )
        else:
            logger.error("Chat list div not found.")
