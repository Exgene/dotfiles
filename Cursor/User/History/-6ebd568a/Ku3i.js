  ...
  const server = http.createServer((req, res) => {
    ...
    switch (pathname) {
      ...
      case "/filterByTitle":
        ...
        const foundBook = bookCollection.find((book) => book.title.toLowerCase() === searchTitle);
        if (foundBook) {
          res.writeHead(200, { "Content-Type": "application/json" });
      ...
      case "/filterByLanguage":
        const searchLanguage = query.language.toLowerCase();
        const filteredBooks = bookCollection.filter(
          (book) => book.language.toLowerCase() === searchLanguage
        );
