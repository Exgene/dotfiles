...
class FeedFoward(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.net = nn.Sequential(
            ...
            nn.ReLU(),
            LoRALinear(
                4 * config.n_embd, config.n_embd, 4, 0.1, 0.01
            ),  # Add LoRA layer
...
class LanguageModelLoRA(nn.Module):
    ...
    @classmethod
    def from_pretrained_with_lora(cls, config, filepath):
    ...
    def from_pretrained(cls, config, filePath):
        ...
        # Load weights for the original layers
        model_dict = model.state_dict()

        # Filter out LoRA layers and print the weights being loaded
        pretrained_dict = {}
        for k, v in state_dict.items():
            # Check if the key belongs to lora_a or lora_b layers
            is_lora_layer = any(
                k.endswith(name) for name in ["lora_a.weight", "lora_b.weight"]
            )

            if k in model_dict and not is_lora_layer:
                print(f"Loading {k}")
                pretrained_dict[k] = v
            else:
                print(f"Skipping {k}")
