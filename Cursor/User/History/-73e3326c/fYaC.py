...
def get_db_path():
    ...
    return os.path.join(current_dir, "..", "data", "main.sqlite")


# Update the get_messages function
...
# Update the insert_message function
def insert_message(
    chatId: int,
    messageId: int,
    provider: str,
...
class WhatsappClient:
    ...
    async def find_user(self, username: str) -> str | bool:
        ...
        try:
            ...
            chat_name = await self.page.locator(
                selector='#main header ._amig span[dir="from src.db import insert_message, get_messagesauto"]'
            ).first.inner_text()
    ...
    async def fetch_and_update_chats(self, users: List[Dict[str, Any]]):
        """
        Fetches chats for specified users and updates the database.

        :param users: List of dictionaries containing user information
                      (e.g., [{"name": "Prathwik", "limit": 10}, {"name": "Kausthubh", "limit": 20}])
        """
        for user in users:
            chats = await self.getChatHistoryByName(user["limit"], user["name"])
            self.update_database(user["name"], chats)

    def update_database(self, user_name: str, chats: List[Dict[str, str]]):
        """
        Updates the database with new chats.

        :param user_name: Name of the user
        :param chats: List of chat messages
        """
        existing_messages = get_messages(user_name)
        existing_message_ids = set((msg[0], msg[1]) for msg in existing_messages)

        new_messages_count = 0
        for chat in chats:
            # Generate a unique messageId (you may need to adjust this based on your needs)
            message_id = int(time.time() * 1000)  # Using timestamp as messageId

            if (user_name, message_id) not in existing_message_ids:
                insert_message(
                    chatId=user_name,
        ...
        print(
            f"Added {new_messages_count} new messages to the database for {user_name}"
        )
