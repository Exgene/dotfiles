...
def get_message_ids(chatId: int):
    """Gets all message IDs for a specific chat."""
    db_path = get_db_path()
    connection = sqlite3.connect(db_path)
    cursor = connection.cursor()

    cursor.execute("SELECT messageId FROM chats WHERE chatId = ?", (chatId,))
    messages = cursor.fetchall()
    connection.close()
    return set(message[0] for message in messages)
...
class WhatsappClient:
    ...
    async def update_database(self, user_name: str):
        """
        ...
        """
        chats = await self.getChatHistory(10)
        for chat in chats:
            message_string = f"{chat['time']}{chat['name']}{chat['message']}"
            hashed_messageId = hashlib.md5(message_string.encode()).hexdigest()
            if hashed_messageId in get_message_ids(user_name):
                print("Message already exists in the database")
                continue
            else:
                insert_message(
                    chatId=user_name,
                    messageId=hashed_messageId,
                    provider="whatsapp",
                    sender=chat["name"],
                    time_sent=chat["time"],
                    message=chat["message"],
                )
                print("Database updated", get_chats_from_db())
